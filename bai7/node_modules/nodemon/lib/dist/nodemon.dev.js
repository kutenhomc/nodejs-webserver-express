...
function nodemon(settings) {
  ...
  config.load(settings, function (config) {
    ...
    config.loaded.map(function (file) {
    ...
    if (config.options.stdin && config.options.restartable) {
      ...
      process.stdin.on('data', function (data) {
        var str = data.toString().trim().toLowerCase(); // if the keys entered match the restartable value, then restart!
      ...
      process.stdin.on('data', function (data) {
        data = data.toString();
        buffer += data;
        var chr = data.charCodeAt(0); // if restartable, echo back
    ...
    if (!config.required) {
      var restartSignal = config.options.signal === 'SIGUSR2' ? 'SIGHUP' : 'SIGUSR2';
      process.on(restartSignal, nodemon.restart);
      utils.bus.on('error', function () {
    ...
    var ignoring = config.options.monitor.map(function (rule) {
      if (rule.slice(0, 1) !== '!') {
        return false;
      }
